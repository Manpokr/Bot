...
del_conf() {
    ...
    uuid="$(cat /usr/local/etc/xray/user.txt | grep -w "$user" | awk '{print $2}')"
...
ext_conf() {
    ...
    if ! grep -qw "$user" /usr/local/etc/xray/user.txt; then
        ...
        uuid=$(grep -wE "$user" "/usr/local/etc/xray/user.txt" | awk '{print $2}')
        ...
        sed -i "/$user/d" /usr/local/etc/xray/user.txt
        echo -e "${user}\t${uuid}\t${exp4}" >>/usr/local/etc/xray/user.txt
